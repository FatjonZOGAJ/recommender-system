...
\begin{document}
    ...
    In our implementation, we incorporated most of these as well, omitting the temporal dynamics due to the absence of this information in our provided dataset.
    The two additions are defined as follows:
    \begin{enumerate}
        \item Implicit User Feature (Bayesian SVD++) $\Leftrightarrow$ all items consumed by user $u$:
        $$w_u=|\{(u,i): (u,i) \in \Omega\}|$$
        \item Implicit Item Feature (Bayesian SVD++ flipped) $\Leftrightarrow$ all users that consumed item $i$:
        $$v_i=|\{(u,i): (u,i) \in \Omega\}|$$
    \end{enumerate}

    In addition to these implicit features, datasets used for benchmarking models such as BFMs usually contain further information, e.g. in the case of movie recommendations, about the genre of a movie, friends of a user, and and many more.
    Given that those pieces of information seem relevant as well, we wanted to incorporate such information.
    Hence, we come up with tw
