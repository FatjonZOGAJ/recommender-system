...
%    In order to provide additional information for models, we implement clustering and that way provide the information about movie categories as in the MovieLens dataset.
    ...
    \begin{algorithm}
        ...
        $k_{opt}=\underset{k}{\argmin}$ RMSE(SVD$_{rank=k}$(data))\\
    ...
    Neural Collaborative Filtering as proposed by He et al.~\cite{DBLP:journals/corr/abs-1708-05031} tries to
    ...
    Firstly, an embedding layer maps each user and each item to its corresponding latent vector representation.
    Output of the embedding layers is then concatenated and passed on through fully connected feedforward layers with ReLU activation functions.
    Finally, output of the network represents the rating associated with the inputted user and item. Network is trained minimizing
    ...
    In \Cref{tab:ablation}, for instance, \textit{SVD*10} stands for chaining SVD ten times after another where after each round we reset the observed entries back to their original values.
    ...
    From \Cref{fig:validation} it is visible that overall the KernelNet performed best, the Autoencoder made continuous progress, while AutoRec and NCF quickly started to diverge.
    ...
    As can be seen in \Cref{tab:ablation}, BFM with implicit user and movie features performed better while standard BFM and BFM with only implicit user features performed worse.
    ...
    Furthermore, the heatmap, illustrated in \Cref{fig:Heatmap}, gives an overview on the conducted parameter search of sampling size per iteration and ranks.
