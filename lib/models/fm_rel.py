...
class FMRelational(BaseModel):
    ...
    def _augment_user_id(self, user_ids, user_id_to_index, movie_id_to_index, user_vs_watched):
        X = sps.lil_matrix((len(user_ids), len(user_id_to_index) + (len(movie_id_to_index) if params.USE_IU else 0)))
    ...
    def _augment_movie_id(self, movie_ids, movie_id_to_index, user_id_to_index, movie_vs_watched):
        X = sps.lil_matrix((len(movie_ids), len(movie_id_to_index) + (len(user_id_to_index) if params.USE_II else 0)))
    ...
    def predict(self, test_movies, test_users, save_submission):
        ...
        unique_user_ids = np.unique(self.df_train.user_id)
        unique_movie_ids = np.unique(self.df_test.movie_id)
        ...
        if params.USE_II:
            feature_group_sizes.append(len(user_id_to_index))  # all users who watched the movies

        # Create RelationBlock.
        train_blocks = []
        test_blocks = []
        for source, target in [(self.df_train, train_blocks), (self.df_test, test_blocks)]:
            unique_users, user_map = np.unique(source.user_id, return_inverse=True)
            target.append(
                RelationBlock(user_map,
                              self._augment_user_id(unique_users, user_id_to_index, movie_id_to_index, user_vs_watched))
            )
            unique_movies, movie_map = np.unique(source.movie_id, return_inverse=True)
            target.append(
                RelationBlock(movie_map, self._augment_movie_id(unique_movies, movie_id_to_index, user_id_to_index,
                                                                movie_vs_watched))
            )
